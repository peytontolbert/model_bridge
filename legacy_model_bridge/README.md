...
```bash
cd /data/legacy_model_bridge
conda activate ai
export PYTHONNOUSERSITE=1
export PYTHONPATH=/data/legacy_model_bridge
```
...
python -m legacy_model_bridge.cli nemo-asr run --archive-path /arxiv/models/parakeet-tdt_ctc-110m/parakeet-tdt_ctc-110m.nemo --inspect-only
...
1. Resolve the local artifact from `/arxiv/models`, `/data/models`, `/data/huggingface/hub`, or a model-specific path.
...
- old environment pin -> latest `ai` package or bridge-owned worker boundary
...
When adding a model, update the bridge code, add or extend focused tests, run a real smoke in `ai`, write the report under `reports/`, then promote the model and patch IDs in the catalog.
